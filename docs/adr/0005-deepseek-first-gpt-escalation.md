...
- Aggregator-Conflict -> GPT-5.6 Sol, `reasoning.effort=high` (signal divergence).
- Macro-Strategist -> GPT-5.6 Sol, `reasoning.effort=xhigh` (capital allocation).

The Router emits `ROUTE_PRO` (DeepSeek-V4-Pro) normally and escalates to `ROUTE_GPT`
(GPT-5.6 Sol) after 4 consecutive conflict ticks, returning after 10 calm ticks. The Circuit
Breaker degrades per model: Flash down -> Text Scouts local filtering (`TEXT_LOCAL_FILTER`);
GPT-5.6 Sol down -> conflicts forced to `WAIT_CONFIRMATION` (`CONFLICT_SAFE`); two or more models
down -> `LOCAL_QUANT_MODE`.

## Consequences
The hot path is cheap and fast; GPT-5.6 Sol is reserved for the highest cost-of-error decisions.
